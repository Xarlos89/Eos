...
class DB:
    ...
    def synchronise(self, discord_client):
        ...
        def is_data_in_db(table_name, column_name, value):
            cursor = db.cursor()
        ...
        def is_member_in_db(member_id):
            return is_data_in_db("members", "member_id", member_id)

        def is_role_in_db(role_id):
            return is_data_in_db("roles", "role_id", role_id)

        def is_channel_in_db(channel_id):
            return is_data_in_db("channels", "channel_id", channel_id)
        ...
        def add_guild_to_db(g_name, g_logo, g_created_at, g_member_count, g_nsfw_level, g_language, dt_now, guild_id):
            cursor = db.cursor()
            query = """INSERT 
                        INTO guilds
                            (guild_id, name, logo, created_at, member_count, nsfw_level, language, last_sync)
        ...
        def add_channel_to_db(guild_id, channel_id, name, category, position, mention, jump_url, permissions_synced,
                              overwrites,
                              created_at, last_synced):
            cursor = create_cursor()
            query = """INSERT 
        ...
        def update_member_info(guild_id, member_id, name, avatar, created_at, nickname, display_name, joined_at):
            ...
            query = """UPDATE 
                        ...
                        """
            cursor.execute(query,
                           (str(guild_id), name, avatar, created_at, nickname, display_name, joined_at, str(member_id)))
            db.commit()
            cursor.close()
        ...
        def sync(discord_client):
            def sync_guild_info():
                for guild in discord_client.guilds:
                    print("- Syncing Guild...")
                    if get_guild_data(guild.id) is None:
                        add_guild_to_db(
                            guild.name
                            , str(guild.icon)
                            , guild.created_at
